...
#answer_file='C:\\Auto-Marking-System\\student2_ans_file.xlsx'

#Create a data frame and get input from AnswerKey
#df=pd.read_excel(answer_key) 
#df=df.set_index('qno')

#Prepare all the StudentAnswer file for marking
for answer_file in glob.glob("C:\\Auto-Marking-System-v2\\student*.xlsx"):
    answer_key='C:\\Auto-Marking-System-v2\\answer_key.xlsx'
    df=pd.read_excel(answer_key) 
    df=df.set_index('qno')
    ...
    for i in df.index:
        ...
        mks=df.loc[i,'marks']
        if df.loc[i,'answer_functions']=="NONE":
            marks=0
            comment="None"
            ...
            if match_ratio < 100 and match_ratio >= 80:
                ...
                comment="Matching Ratio betweeb 80-99"
            else:
